...
testAnnotationConnectionButton.addEventListener('click', async function (e) {
  var serverUrl = annotationServerUrlInput.value.trim()
  var username = annotationUsernameInput.value.trim()
  var password = annotationPasswordInput.value.trim()
  ...
  testAnnotationConnectionButton.disabled = true
  testAnnotationConnectionButton.textContent = 'Testing...'
  
  try {
    // Step 1: Test server availability with meta/version endpoint
    console.log('Testing server availability...')
    var versionResponse = await fetch(serverUrl + '/meta/version', {
      method: 'GET',
      headers: {
    ...
    if (!versionResponse.ok) {
      throw new Error(`Server not responding (HTTP ${versionResponse.status})`)
    }
    
    var versionData = await versionResponse.json()
    if (!versionData.notelix) {
      throw new Error('Server is not a valid Notelix server')
    }
    
    console.log('Server is available and valid')
    
    // Step 2: Test authentication with login endpoint
    console.log('Testing authentication...')
    var loginResponse = await fetch(serverUrl + '/users/login', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    // Step 3: Test API access with who-am-i endpoint
    console.log('Testing API access...')
    var whoAmIResponse = await fetch(serverUrl + '/users/who-am-i', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (!whoAmIResponse.ok) {
      throw new Error(`API access failed (HTTP ${whoAmIResponse.status})`)
    }
    
    var whoAmIData = await whoAmIResponse.json()
    if (!whoAmIData.id || !whoAmIData.name) {
      throw new Error('Invalid user data received from server')
    }
    
    console.log('API access successful')
    
    // Step 4: Test annotation endpoint
    console.log('Testing annotation access...')
    var annotationsResponse = await fetch(serverUrl + '/annotations/list', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'jwt ' + loginData.jwt
    ...
    if (!annotationsResponse.ok) {
      throw new Error(`Annotations access failed (HTTP ${annotationsResponse.status})`)
    }
    
    var annotationsData = await annotationsResponse.json()
    if (!annotationsData.hasOwnProperty('list')) {
      throw new Error('Invalid annotations response from server')
    }
    
    console.log('Annotation access successful')
    
    // Save JWT token for future use
    settings.set('annotationJwt', loginData.jwt)
    
    // Success message
    var annotationCount = annotationsData.list ? annotationsData.list.length : 0
    alert('✅ Connection test successful!\n\n' +
          'Server: ' + serverUrl + '\n' +
          'User: ' + whoAmIData.name + ' (ID: ' + whoAmIData.id + ')\n' +
          'Existing annotations: ' + annotationCount + '\n\n' +
          'Authentication token has been saved.')
    ...
    if (error.message.includes('Failed to fetch') || error.message.includes('Network request failed')) {
      ...
      errorMessage += '\n\n💡 Troubleshooting:\n' +
                     ...
                     '• Make sure the account exists on the server\n' +
                     '• Check if the account is active'
    } else if (error.message.includes('not a valid Notelix server')) {
      errorMessage += '\n\n💡 Troubleshooting:\n' +
                     '• Ensure you\'re connecting to a Notelix server\n' +
    ...
    testAnnotationConnectionButton.disabled = false
    testAnnotationConnectionButton.textContent = 'Test Connection'
  }
})
