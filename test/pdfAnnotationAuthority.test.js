...
test('pdf annotation IPC enforces the viewer, tab, frame, source, and session authority', async t => {
  ...
  const annotations = path.join(root, 'Archives', 'Annotations')
...
test('root change admits only the prepared PDF Save to the old vault before switching', async t => {
  ...
  assert.equal(result.ok, true, result.error)
  assert.equal(fs.readdirSync(path.join(root, 'Archives/Annotations')).length, 1)
  assert.equal(fs.existsSync(path.join(nextRoot, 'Archives/Annotations')), false)
})
