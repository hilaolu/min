...
const PDFViewer = {
  ...
  isPDFViewer: function (tabId) {
    return browserSession.tabs.get(tabId).url.startsWith(PDFViewer.url.base)
  },
  ...
  handlePDFOpenEvent: function (event, data) {
    if (!data.tabId) {
      var matchingTabs = browserSession.tabs.get().filter(t => urlParser.getSourceURL(t.url) === data.url).sort((a, b) => { return b.lastActivity - a.lastActivity })
      if (matchingTabs[0]) {
        data.tabId = matchingTabs[0].id
      }
    }
    if (!data.tabId) {
      console.warn('missing tab ID for PDF', data.url, browserSession.tabs.get().map(t => t.url))
      return
    }
