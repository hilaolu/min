/**
 * Open URL State Strategy
 * 
 * Handles ">o [url]" command - opens URLs with history/bookmark candidates
 */

const { CommandStateStrategy } = require('../CommandStateStrategy.js')

class OpenUrlStrategy extends CommandStateStrategy {
  constructor() {
    super('OPEN_URL', 80) // High priority for specific commands
  }
  ...
  matches(input) {
    ...
    // Matches ">o" optionally followed by space and arguments
    const match = trimmed.match(/^>o(\s+(.*))?$/i)
    
    if (match) {
      const args = match[2] || ''
      return {
        matches: true,
        data: { command: 'o', args: args.trim() },
  ...
  /**
   * Generate URL candidates from history and bookmarks
  ...
  async generateCandidates(searchQuery = '') {
    try {
      ...
      if (searchQuery && searchQuery.trim()) {
        const candidate = this.createURLCandidate(searchQuery)
        if (candidate) {
      ...
      candidates = candidates.concat(results.map(result => this.createHistoryCandidate(result)))

      // Limit to 10 candidates for keyboard shortcuts (0-9)
      return candidates.slice(0, 10)
    } catch (error) {
      console.error('Error generating open candidates:', error)
      return []
    }
  }
   ...
   */
  getPlaceholder(data = {}) {
    return data.args ? `Open URL: ${data.args}...` : 'Enter URL to open...'
  }
  ...
  renderCandidates(candidates, context, searchQuery = '') {
    ...
    if (candidates.length === 0) {
      ...
      emptyEl.textContent = searchQuery ? 'No matching URLs found' : 'Enter URL to open'
  ...
  /**
   * Create a suggestion element for URL candidates
  ...
  createSuggestionElement(candidate, index, context, searchQuery) {
    ...
    suggestionEl.className = 'command-suggestion url-candidate'
...
module.exports = OpenUrlStrategy 